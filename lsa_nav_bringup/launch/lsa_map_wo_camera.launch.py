...
def generate_launch_description():
    ...
    lsa_map_pkg = get_package_share_directory('lang_sam_to_map')

    # RViz設定ファイルパス
    rviz_config = os.path.join(lsa_map_pkg, 'rviz', 'rviz.rviz')
    
    # パラメータファイルパス
    param_dir = os.path.join(pkg_dir, 'param', 'param.map_scan.yaml')
    ...
    load_nodes = GroupAction(
        actions=[
            ...
            # map integrator
            Node(
                package='map_scan_integrator',
                executable='map_scan_integrator',
                name='map_scan_integrator',
                output='screen',
